...
router.get('/my-orders', requireAuth, async (req, res) => {
  try {
    console.log('My Orders route accessed by user:', req.session.userId);
    ...
    console.log(`Found ${userOrders.length} orders for user ${currentUser.email}`);

    res.render('my-orders', {
      title: 'My Orders - Hobbs',
      orders: userOrders,
      currentPage: page,
    ...
    res.render('my-orders', {
      title: 'My Orders - Hobbs',
      orders: [],
      error: 'Error loading your orders. Please try again.',
      currentPage: 1,
      totalPages: 1,
      hasNextPage: false,
      hasPrevPage: false,
...
router.post('/place', async (req, res) => {
  try {
    ...
    const orderData = {
      ...
      customerEmail: email ? email.trim() : (req.session.userEmail || undefined),
...
router.get('/:orderId', async (req, res) => {
  try {
    ...
    res.render('order-detail', {
      title: `Order ${order.orderNumber} - Hobbs`,
      order: order
    });
  } catch (error) {
